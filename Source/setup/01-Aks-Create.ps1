...
Write-Host "------------------------------------------------------------"
$outputs=(az group deployment create -n $env:AKS_NAME -g $env:AKS_RG --template-file smarthote360.backend.deployment.json --parameters servicePrincipalClientId=$env:SPN_CLIENT_ID servicePrincipalClientSecret=$env:SPN_PW)
