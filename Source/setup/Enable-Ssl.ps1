Param (
    [parameter(Mandatory=$false)][string][ValidateSet('prod','staging', IgnoreCase=$false)]$sslSupport = "staging",
    [parameter(Mandatory=$false)][string]$name = "$env:SH360_APPNAME",
    [parameter(Mandatory=$false)][string]$aksName = "$env:AKS_NAME",
    [parameter(Mandatory=$false)][string]$resourceGroup = "$env:AKS_RG"
)
